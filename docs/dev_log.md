...
* **Модульные тесты:** проверяют отдельные функции изолированно от внешней среды. Например, `tests/test_analyzer.py` тестирует алгоритм `analyze_market_trend()` без обращения к реальной базе данных. Для этого зависимость `get_recent_prices()` подменяется через `monkeypatch`.
